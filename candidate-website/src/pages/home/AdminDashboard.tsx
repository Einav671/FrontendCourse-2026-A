...
import './Home.css'; // נשתמש באותו עיצוב בינתיים

const AdminDashboard: React.FC = () => {
    const navigate = useNavigate();
    const [alerts, setAlerts] = useState<SystemAlert[]>([]);
    const [stats, setStats] = useState({
        leads: 0, candidates: 0, scholarships: 0, pendingGraduates: 0
    });
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            try {
                ...
                const alertsData = alertsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as SystemAlert));
    ...
    const renderKpiCard = (title: string, value: string | number, subtext: string, icon: React.ReactNode, bgColor: string, color: string, path: string) => (
        <Grid>
            <Paper elevation={2} className="kpi-paper" onClick={() => navigate(path)}>
                <Avatar sx={{ width: 56, height: 56, bgcolor: bgColor, color: color }}>{icon}</Avatar>
                <Box className="kpi-content">
                    <Typography variant="subtitle1" color="text.secondary" fontWeight={500}>{title}</Typography>
                    <Typography className="card-value">{loading ? <CircularProgress size={20} /> : value}</Typography>
            ...
            <Grid container spacing={4}>
                <Grid >
                    <Paper elevation={3} className="alerts-section-paper">
                        <Typography variant="h6" className="section-title">התראות מערכת</Typography>
                        <Stack spacing={2}>
                            {loading ? <CircularProgress /> : alerts.length > 0 ? alerts.map((alert) => (
                                <Box key={alert.id} className={`alert-item alert-${alert.type}`}>
                                    {alert.message} {getAlertIcon(alert.type)}
                                </Box>
                            )) : <Typography className="no-alerts-text">אין התראות חדשות</Typography>}
                        </Stack>
                    </Paper>
                </Grid>
            </Grid>
